...
double test(vector<string> allWords, vector<string> queries,promise<bool> done_future)
{
    ...
    for(int i = 0; i < allWords.size(); i++)
    {
        if (!myTable->lookup(allWords[i]))
        {
            cout << "Failed!" << endl;
            return score;
        }

        if (myTable->lookup(allWords[i])->value != allWords[i])
        {
            cout << "Failed!" << endl;
            return score;
        }
    ...
    for(int i = 0; i < queries.size()/2; i++)
    {
        if (myTable->lookup(queries[i]) != NULL)
        {
            cout << "Failed!" << endl;
            return score;
        }
    ...
    {
        if (!myTable->lookup(queries[i]))
        {
            cout << "Failed!" << endl;
            return score;
        }

        if (myTable->lookup(queries[i])->value != queries[i])
        {
            cout << "Failed!" << endl;
            return score;
        }
...
int main()
{
    ...
    cout << "Total Score: " << score << "/40" << endl;
